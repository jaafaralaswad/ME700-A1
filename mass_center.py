...
def mass_center(n: int, tol_input: float, tol_output: float, max_iterations: int) -> dict:
    ...
    # Arrange in an ascending order according to position

    # Combine masses and positions into a list of tuples
    combined = list(zip(positions, masses))

    # Sort the combined list based on positions (first element of the tuple)
    combined.sort()

    # Unpack the sorted positions and masses
    positions, masses = zip(*combined)
    ...
    fnc = lambda x: sum(m * (x_i - x) for m, x_i in zip(masses, positions))
...
def validate_interval(a: float, b: float, fnc_a: float, fnc_b: float):
    ...
    if fnc_a*fnc_b < 0:
        ...
        raise ValueError("A root in interval [{a},{b}] is not guaranteed. Make sure you entered correct values.")                  
    return
